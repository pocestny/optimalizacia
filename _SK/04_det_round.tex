...
$V'(\rho)=C(\rho)$ (porovnajte $C(\rho)$ a členy $V(\rho)$ ktoré sú závislé od $\rho$). Preto
...
iterácií skončí. Ľahko vidno, že po skončení je každá úloha priradená nejakému procesoru.

Čo môže byť výstupom algoritmu? Povedzme, že pre nejaký problém \mingap je pre $T=10$ minimálna dosiahnuťelná cena $OPT=100$ a pre $T=20$ zase $OPT=70$. Keď pustíme algoritmus pre $T=10$, môže sa stať, že dostaneme riešenie s cenou $ALG=97$ ktoré na dokončenie potrebuje čas $T=16$.

Ostáva nám ukázať, že toto priradenie má optimálnu cenu a termín je prekročený najviac o dvojnásobok:
